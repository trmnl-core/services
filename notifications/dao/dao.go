...
var (
	defaultStore = mstore.DefaultStore
)

// Init replaces the store
...
func writeObj(obj interface{}, key string) error {
	...
	if err := defaultStore.Write(&store.Record{
		Key:   key,
		Value: b,
	}); err != nil {
		return err
...
func ListNotificationsForSubscriber(subscriberID string) ([]*Notification, error) {
	entries, err := defaultStore.Read(fmt.Sprintf(prefixNotificationsBySubscriber, subscriberID), store.ReadPrefix())
	if err != nil {
		return nil, err
	}
...
func ReadNotification(subscriberID, notifID string) (*Notification, error) {
	entries, err := defaultStore.Read(fmt.Sprintf(keyNotificationsBySubscriber, subscriberID, notifID))
	if err != nil {
		return nil, err
	}
	ret := &Notification{}
...
func ListSubscriptionsForResource(resourceType, resourceID string) ([]*Subscription, error) {
	entries, err := defaultStore.Read(fmt.Sprintf(prefixSubscriptionsByResource, resourceType, resourceID), store.ReadPrefix())
	if err != nil {
		return nil, err
	}
...
func DeleteSubscription(subscriptionID, subscriberID, resourceType, resourceID string) error {
	...
	if err := defaultStore.Delete(fmt.Sprintf(keySubscriptionsByResource, resourceType, resourceID, subscriberID)); err != nil {
		return err
	}
	return defaultStore.Delete(fmt.Sprintf(keySubscriptionsBySubscriber, subscriberID, subscriptionID))
}
...
func ReadSubscription(subscriberID, resourceType, resourceID string) (*Subscription, error) {
	entries, err := defaultStore.Read(fmt.Sprintf(keySubscriptionsByResource, resourceType, resourceID, subscriberID))
	if err != nil {
		return nil, err
	}
	ret := &Subscription{}
...
func ListSubscriptionsForSubscriber(subscriberID string) ([]*Subscription, error) {
	entries, err := defaultStore.Read(fmt.Sprintf(prefixSubscriptionsBySubscriber, subscriberID), store.ReadPrefix())
	if err != nil {
		return nil, err
	}
	ret := make([]*Subscription, len(entries))
