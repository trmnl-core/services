...
func NewTestsService(name string) TestsService {
	return &testsService{name: name}
}

func (c *testsService) Register(ctx context.Context, in *RegisterRequest, opts ...client.CallOption) (*RegisterResponse, error) {
	req := microClient.NewRequest(c.name, "Tests.Register", in)
...
func RegisterTestsHandler(hdlr TestsHandler, opts ...server.HandlerOption) error {
	...
	h := &testsHandler{hdlr}
	return microServer.Handle(microServer.NewHandler(&Tests{h}, opts...))
}
